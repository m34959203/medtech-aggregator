...
## Чек-поинт 2026-06-28 (#56) — редизайн «МедЦена» (Claude Design import)
- Импортирован дизайн из claude.ai/design (проект «Redesign medtech site», `MedTsena.dc.html`) через DesignSync MCP (get_file).
- **Ретема токенов** (главный рычаг — переодевает всё приложение): `tailwind.config.ts` brand→тёплая зелёная (#0F8A7E/#0B6F66/#E3F1EE), ink→зелёно-серый (#16241F/#57655F/#9AA8A2/#E4EAE7), тени зелёно-тонированные; `globals.css` `--background:#F4F7F5`; `layout.tsx` шрифт **Golos Text** (через var --font-inter).
- **Цветные категории** (`CategoryBadge`): лаборатория=зелёный, приём=золотой, диагностика=синий, процедура=фиолет. (regex по имени категории) — разом обновляет карточки и деталь.
- **ServiceCard**: бейдж экономии «−N%» (≥5%), цена 22px extrabold.
- **ComparisonView**: стат-полоса (минимальная=зелёная карточка / средняя / разница цен).
- Logo (зелёный квадрат+крест) и Hero-копирайт уже совпадали с макетом.
- Verify: tsc 0, build ок, ребилд frontend. Прод: Golos в разметке, `#0F8A7E/#16241F/#F4F7F5` в CSS-бандле.
- **Долг (точечная доводка под макет 1:1)**: hero лево-выровнен с инлайн-поиском; фильтр категорий чипами (сейчас селекты); ценовые бары в офферах детали; кнопка «Записаться» тёмная→зелёная hover. Функционал весь сохранён.
...
## Чек-поинт 2026-06-28 (#58) — дедуп очереди ручной проверки + витрина без записи
- **Убрана форма записи с витрины** (`ComparisonView.tsx`): удалён `LeadButton` (Имя/Телефон → «Оставить заявку») с ценовых офферов; в чат-виджете CTA «Сравнить и записаться» → «Сравнить цены». Бэкенд `POST /api/leads` + таблица `leads` оставлены СПЯЩИМИ (по решению Дмитрия). Коммит `61807ea`.
- **Разбор очереди «на проверке» 7 785**: оказалось = цены с `service_id IS NULL`. Из них только 4 199 уникальны по (клиника+сырое имя), ~3 586 — дубли повторных загрузок.
- **Откат дублей**: сгруппировал прогоны по клинике + ядру нераспознанных имён (md5 distinct raw_name where service_id is null), в каждой группе оставил прогон с макс. сопоставлением, остальные откатил через `POST /api/ingest/runs/{id}/rollback`. Откатано **16 прогонов / 2 520 строк** (6 групп: keep 1905/1750/1276/1746/1669/967). Остаточных дубль-групп — **0**.
- **Результат**: цены 14 699→12 258, очередь проверки **7 785→5 425**, авто-доля 47%→**55.7%**, аномалии 393→218. Оставшиеся 5 425 — реальные промахи матчера, не мусор.
- **[ДОРАБОТКА — корень проблемы, НЕ сделано]** Идемпотентности на загрузке НЕТ: `_dedup` чистит только внутри одного файла (`web_scraper.py`/`archive_service.py`), а повторная загрузка/переобработка того же файла НЕ проверяется — у `ingestion_runs` есть `file_name`/`file_path`, но НЕТ content-hash и нет проверки «такой прогон уже был» → каждый повтор кладёт полную копию (1274→1275→1276). Это долг #54 п.1, подтверждён на данных.
  - **Вариант A (предпочт.)**: upsert по ключу `(clinic_id, raw_name)` — повтор обновляет цену/версию, а не плодит строку.
  - **Вариант B**: хэш содержимого файла в `ingestion_runs` + отказ/замена при совпадении.
  - До фикса дубли будут копиться снова; временное лечение — этот же rollback-проход.
