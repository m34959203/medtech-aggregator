...
- **Долг**: фронт фичи сравнения (чекбоксы «Сравнить» 2–4 клиник + закреплённая панель + таблица) — разведано, инфраструктура (`/compare`, `ComparisonView`, поля API) почти готова; не реализовано (приоритет был на достоверность). Distance-«чекпоинт» на главной/предложениях — backend готов (haversine), нужен персист геолокации на фронте.
