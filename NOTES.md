...
- Готчи нормализации ОАК (2026-06-17, баг «ОАК только 1 клиника в Караганде»): (1) лаборатории зовут ОАК «Клинический анализ крови» — добавлен ключ; (2) многопрофильные **клиники** курировались только приёмами/УЗИ — теперь `CLINIC_KEYWORDS + LAB_KEYWORDS` (клиники отдают и анализы); (3) ключ «соэ» ловил standalone-СОЭ, который без LLM ложно мержился в ОАК (выброс 200₸) — ключ убран. Итог: ОАК 1→5 клиник в Караганде, 8→36 предлож. по стране, 487→599 цен.
