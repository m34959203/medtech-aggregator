...
- **LLM — Gemini 2.5-flash** (через Vertex AI) с фолбэком на **AlemLLM/Groq**. Провайдер переключается через `LLM_PROVIDER` — один и тот же код. Tool-calling намеренно не используется (retrieval-injection надёжнее и провайдер-агностичен).
