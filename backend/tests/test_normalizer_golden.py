...
def db():
    ...
    cat = [
        ...
        # КОНКУРЕНТ для тай-брейка: «ТТГ» — подмножество и атомарной услуги, и
        # длинного «Антитела к рецепторам ТТГ»; token_set даёт обоим 100%.
        ServiceCatalog(canonical_name="Антитела к рецепторам ТТГ", category="Анализы", synonyms=[]),
        ServiceCatalog(canonical_name="Холестерин общий", category="Анализы", synonyms=[]),
        ServiceCatalog(canonical_name="Холестерин", category="Анализы", synonyms=[]),
    ]
    s.add_all(cat)
    s.commit()
    yield s
...
MATCHED = [
    ...
    ("HBsAg", "HBsAg"),
    # тай-брейк: короткое сырьё не должно «прилипать» к длинной услуге-надмножеству
