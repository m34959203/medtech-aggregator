...
В проде (Postgres, на 2026-06-26): **99 клиник · 613 услуг · 2812 цен · 601 услуга с ценой · 14 городов** (Алматы/Астана/Караганда/Актобе/Шымкент/Актау/Семей/Павлодар/Петропавловск/Усть-Каменогорск/Тараз/Костанай/Талдыкорган/Кызылорда).
...
|---|---|---|
| Приведение разнородных названий к единому справочнику | ✅ | `ingestion/normalizer.py`: fuzzy (token_set) → семантика (эмбеддинги/pgvector) → **LLM-арбитраж (Gemini 2.5-flash / Vertex)** + guard'ы (кровь/моча, варианты) |
...
| Справочник формируется командой на основе собранных данных | ✅ | **619 услуг** из реальных источников; командой доведены пробелы (АСТ, ПСА, Т4/Т3 свободный/общий) |
...
| Сравнение таблицей | 🟡 список-сравнение клиник + «рецепт» `/recipe` (минимум по услугам/одна клиника) |
| Подписка на изменение цены | ⛔ не реализовано (опционально) |
| Маршрут до клиники | 🟡 кнопка в балуне карты «отправить адрес в WhatsApp» с маршрутной ссылкой Яндекс.Карт (навигатор) |
...
|---|---|
| Рабочий MVP + README | ✅ |
| БД с реальными данными (≥3 источника, ≥100 услуг) | ✅ **99 клиник, 613 услуг (601 с ценой), 2812 цен, 14 городов** — кратно превышает минимум (≥3/≥100) |
| Справочник ≥50 нормализованных | ✅ **613 позиций** справочника |
