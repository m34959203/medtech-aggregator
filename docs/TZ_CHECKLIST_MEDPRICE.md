...
| Отправить координаты в WhatsApp (сверх ТЗ) | ✅ в балуне метки карты — React-модалка ввода номера → `wa.me/<номер>` с адресом, координатами (lat,lng), точкой на карте и маршрутом |
