...
Основной провайдер — **Gemini 2.5-flash через Vertex AI** (единая точка `llm.chat`,
та же, что у нормализатора/recheck); фолбэк — OpenAI-совместимые AlemLLM/Groq через
`LLM_PROVIDER`. Tool-calling не используется намеренно: retrieval-injection
провайдер-агностичен и исключает галлюцинации цен. Без доступного провайдера endpoint
деградирует в детерминированный поиск-сводку (`_fallback`) — демо работает всегда.

**OCR в чате** (`POST /api/chat/vision`): пациент шлёт фото/скан направления → OCR
(tesseract rus/kaz/eng, тот же `_extract_text_any`, что у `/recipe`) → распознанные
услуги нормализуются и проходят тот же retrieval-injection. Так OCR задействован в
трёх точках: архивные сканы (Кейс 2), страница `/recipe`, чат-помощник.
